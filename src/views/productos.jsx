...
const Productos = () => {
  ...
  const actualizarProducto = async (productoActualizado) => {
    if (
      ...
      !productoActualizado?.descripcion_producto ||
    ...
    try {
      const respuesta = await fetch(
        `http://localhost:3000/api/actualizarproductos/${productoActualizado.id_producto}`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            ...
            descripcion_producto: productoActualizado.descripcion_producto,
            id_categoria: productoActualizado.id_categoria,
            precio_unitario: productoActualizado.precio_unitario,
            stock: productoActualizado.stock,
            imagen: productoActualizado.imagen // Incluir imagen
          }),
        }
      );
      ...
      await obtenerProductos();
      setMostrarModalEdicion(false); // Cerrar el modal tras éxito
  ...
  const abrirModalEdicion = (producto) => {
    console.log('Abriendo modal de edición para producto:', producto); // Depuración
